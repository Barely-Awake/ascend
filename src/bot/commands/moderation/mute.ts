...
export default class Mute implements CommandClass {
  ...
  async command(message: Message, args: string[]) {
    ...
    if (user == null)
      return message.reply('Could not find target user.');

    const guildMember = await message.guild!.members.fetch(user.id);

    if (!guildMember)
      return message.reply('Could not find target user in this guild. Are they a member?');

    let duration: number | null;
    try {
      duration = args[0] ? ms(args[0]) : null;

      if (duration === null ? false : isNaN(duration))
        duration = null;
    } catch (err) {
      if (err === 'Value exceeds the maximum length of 100 characters.')
        duration = null;
      else
        return message.reply('Please provide a valid duration, for example: `1d` (1 day).');
    }
    ...
    if (!guildData || !guildData.muteRole)
      return message.reply('Please set up a mute role using the `muterole` command');
    ...
    await message.reply(`Successfully muted ${guildMember.toString()}${reason ? ` for ${reason}` : ''}`);
  }
}
